...
public class WolfSSLSessionTest {
    ...
    public void test_WolfSSLSession_UseAfterFree()
        ...
        try {
            /* Create ServerSocket first to get ephemeral port,
             * set 10 sec timeout on accept() to prevent test hangs. */
            ...
            /* Flush session cache to avoid interference from other tests.
             * Native wolfSSL session cache is static and shared.
            srvCtx.flushSessions(-1);
            cliCtx.flushSessions(-1); */
        ...
        try {
            /* this should fail, use after free - ssl has been freed above */
            ret = ssl.connect();
    ...
    public void test_WolfSSLSession_getSessionID()
        ...
        try {
            ...
            /* Test getSessionID() - for TLS 1.2 connections, a session ID
             * should always be present after successful handshake */
            sessionID = ssl.getSessionID();
    ...
    public void test_WolfSSLSession_getSetSession() throws Exception {
        ...
        try {
             ...
             * timeout on accept() to prevent test hangs. */
            srvSocket = new ServerSocket(0);
            srvSocket.setSoTimeout(10000);
            final ServerSocket finalSrvSocket = srvSocket;
            ...
            es.submit(new Callable<Void>() {
                ...
                public Void call() throws Exception {
                    ...
                    try {
                        ...
                        for (int i = 0; i < 2; i++) {
                            /* Signal ready before first accept */
                            if (i == 0) {
                                serverReady.countDown();
                            }
                            server = finalSrvSocket.accept();
    ...
    @Test
    public void test_WolfSSLSession_readWriteBoundsCheck() throws Exception {
        int ret = 0;
        int err = 0;
        Socket cliSock = null;
        WolfSSLSession cliSes = null;
        ...
        cliCtx = createAndSetupWolfSSLContext(cliCert, cliKey,
            WolfSSL.SSL_FILETYPE_PEM, caCert, WolfSSL.SSLv23_ClientMethod());
            ...
            es.submit(new Callable<Void>() {
                ...
                public Void call() throws Exception {
                    ...
                    try {
                        ...
                        if (ret != WolfSSL.SSL_SUCCESS) {
                            throw new Exception(
                                "WolfSSLSession.accept() failed: " + err);
                        }
        ...
        synchronized (byteBufferPoolPropertyLock) {
            ...
            /* Disable ByteBuffer pool so native byte[] path is used */
            String originalProp = Security.getProperty(
                "wolfssl.readWriteByteBufferPool.disabled");
            Security.setProperty(
            ...
            try {
                ...
                /* Test write with offset + length > array length.
                 * Array is 17 bytes, offset=5 length=100 should be rejected */
                ret = cliSes.write(testData, 5, 100, 0);
                if (ret >= 0) {
                    throw new Exception("write() with out-of-bounds offset/" +
                        "length should have failed but got: " + ret);
                }

                /* Test read with offset + length > array length */
                byte[] readBuf = new byte[10];
                ret = cliSes.read(readBuf, 5, 100, 0);
                if (ret >= 0) {
                    throw new Exception("read() with out-of-bounds offset/" +
                        "length should have failed but got: " + ret);
                }
                ...
                Security.setProperty(
                    "wolfssl.readWriteByteBufferPool.disabled", originalProp);
