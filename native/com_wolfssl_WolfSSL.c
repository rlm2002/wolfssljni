...
JNIEXPORT jint JNICALL Java_com_wolfssl_WolfSSL_init
  (JNIEnv* jenv, jobject jcl, jint skipFIPSCAST)
{
    int ret = 0;
     ...
     * multi threaded issues later on. Can be skipped if application has
     * already run CASTs through another path,
     * eg. wolfJCE Fips.runAllCast_fips() */
    if (skipFIPSCAST == 0) {
